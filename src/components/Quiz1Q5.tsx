...
import Quiz1CompletePopup from './Quiz1CompletePopup';
...
const Quiz1Q5: React.FC<Quiz1Q5Props> = ({ onBack, onComplete, onAnswer }) => {
  ...
  // Debug log
  useEffect(() => {
    console.log('Quiz1Q5 mounted');
    return () => console.log('Quiz1Q5 unmounted');
  }, []);

  return (
    <div className="h-screen w-full flex flex-col bg-[#FFDE3D] relative overflow-hidden">
      {/* Removed the popup from here since it's now handled by the parent */}
      {/* Notification Bar */}
            ...
            {questions.map((question) => (
              <div key={question.id} className="space-y-4">
                <div className="space-y-2">
                  <div className="text-black text-base font-medium space-y-3">
                    {question.context}
                  </div>
